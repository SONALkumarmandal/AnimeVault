  ...
  const fetchData = async () => {
    try {
      ...
      const trendings = animeList
        ...
        .sort((a, b) => a.rank - b.rank) // Sort by score
        .slice(0, 3); // Take top 3
  
      // console.log("Filtered Trending Anime:", trendings);
      setTrending(trendings);
      
      const topTens = animeList.slice(0, 20); // Take top 10
      setTopTen(topTens);
  ...
  {Array.isArray(trending) && trending.length > 0 ? (
    trending.map((item,ind) => (
      <li className="flex justify-around w-full h-[70vh] align-middle p-6 gap-4 bg-zinc-900 rounded-xl text-nowrap" key={item.mal_id}><div>
        ...
        <strong className="text-orange-700 h-1/3">{item.title_english===null?item.title:item.title_english}</strong> <br />  <p>rank: {item.rank}</p> <div>⭐ {item.score}</div>
     ...
     <div className="h-auto w-full bg-zinc-800 flex justify-center items-center mt-40 text-center">
      ...
      <li className="flex w-full h-auto p-4 gap-4 bg-zinc-900 rounded-xl text-nowrap " key={item.mal_id}><div>
        <div className="flex h-full w-full mt-8 p-8"><img className="rounded-lg h-72 w-auto object-cover" src={`${item.images.webp.image_url}`} alt="img" />
          <strong className="text-orange-700 h-1/3 p-8 pt-0 flex-col text-start text-wrap content-center"><div className="font-medium text-5xl font-mono text-slate-300 "><i>{ind+1}</i></div>{item.title_english===null?item.title:item.title_english}
          <div className="text-white text-start">Favorites: {item.favorites}</div>
          <div className="text-white">⭐ {item.score}</div>
          <div className="text-white text-start text-wrap font-light pt-1">{item.synopsis}</div>
          </strong>
        </div>
      </div>
      </li>
